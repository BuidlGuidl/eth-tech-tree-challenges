...
contract MockToken is ERC20 {
    constructor(uint256 _initialSupply) ERC20("Mock Token", "MOCK") {
        _mint(msg.sender, _initialSupply);
    }
}

contract TokenStreamingTest is Test {
    ...
    function setUp() public {
        ...
        if (keccak256(abi.encodePacked(contractPath)) != keccak256(abi.encodePacked("none"))) {
            bytes memory args = abi.encode(FREQUENCY, address(token));
            bytes memory bytecode = abi.encodePacked(vm.getCode(contractPath), args);
            address deployed;
            assembly {
                deployed := create(0, add(bytecode, 0x20), mload(bytecode))
            }
            tokenStreaming = TokenStreaming(deployed);
        } else {
            tokenStreaming = new TokenStreaming(FREQUENCY, address(token));
        }
        
        // Fund the contract with tokens
        token.transfer(address(tokenStreaming), STARTING_BALANCE);
        
        // Pass time to simulate what it's like to deploy on network that is not brand new
        vm.warp(STARTING_TIMESTAMP);
        // Add a stream for ALICE
        tokenStreaming.addStream(ALICE, STREAM_CAP);
    }
    ...
    function testOnlyOwnerCanAddStream() public {
        ...
        vm.expectRevert();
        tokenStreaming.addStream(ALICE, 333);
    }
    ...
    function testOwnerCanAddStream() public {
        ...
        emit TokenStreaming.AddStream(ALICE, STREAM_CAP);
        tokenStreaming.addStream(ALICE, STREAM_CAP);
    }
    ...
    function testWithdrawCannotExceedBalance() public {
        ...
        tokenStreaming.addStream(BOB, amount);
        vm.expectRevert();
        vm.prank(BOB);
        tokenStreaming.withdraw(amount);
    }
    ...
    function testInvalidAccountCannotWithdraw() public {
        vm.prank(BOB);
        vm.expectRevert();
        tokenStreaming.withdraw(STREAM_CAP);
    }
    ...
    function testValidAccountCanWithdraw() public {
        vm.prank(ALICE);
        vm.expectEmit(true, false, false, true);
        emit TokenStreaming.Withdraw(ALICE, STREAM_CAP);
        tokenStreaming.withdraw(STREAM_CAP);
    ...
    function testValidAccountCanWithdrawPartialAmountsOfUnlocked() public {
        vm.prank(ALICE);
        tokenStreaming.withdraw(STREAM_CAP / 2);
        vm.prank(ALICE);
        tokenStreaming.withdraw(STREAM_CAP / 2);
        uint aliceBalanceAfter = token.balanceOf(ALICE);
    ...
    function testOwnerCanUpdateStream() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit TokenStreaming.AddStream(ALICE, newCap);
        tokenStreaming.addStream(ALICE, newCap);
        vm.prank(ALICE);
        vm.expectEmit(true, false, false, true);
        emit TokenStreaming.Withdraw(ALICE, newCap);
        tokenStreaming.withdraw(newCap);
    }
} 
